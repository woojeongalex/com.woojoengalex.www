...
export const VOCAL_DROPZONE_COPY: MediaDropzoneCopy = {
  idleResetStatus:
    '곡을 선택한 뒤 마이크 녹음 또는 영상·음원 파일을 끌어다 놓으세요.',
  fileReady: (name) =>
...
export function instrumentDropzoneCopy(
...
export function MediaAnalysisDropzone({
  ...
  const acceptFile = useCallback(
    (next: File) => {
      ...
      if (!isAnalyzableMedia(next)) {
        patch({
          errorMessage:
            'MP4, WebM, MOV, MP3, WAV 등 영상·음원만 올릴 수 있습니다.',
          phase: 'error',
        })
        return
      }
  ...
  const isVideo =
    Boolean(drop.file?.type.startsWith('video/')) ||
        ...
        >
          ...
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-zinc-100">
            {drop.phase === 'analyzing' ? (
              <Loader2
                className="h-6 w-6 animate-spin text-zinc-700"
                aria-hidden="true"
              />
            ) : (
