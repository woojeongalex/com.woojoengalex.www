...
async function searchLocal(query: string): Promise<Contact[]> {
  const res = await fetch(`${API_BASE}/api/the-wire/contacts?q=${encodeURIComponent(query)}`)
  if (!res.ok) return []
...
async function searchGoogle(query: string, accessToken: string): Promise<Contact[]> {
