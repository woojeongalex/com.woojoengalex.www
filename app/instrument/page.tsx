      ...
      >
        {messages.length === 0 && !loading && (
          <p className="text-xs text-muted-foreground">
            튜닝 결과를 바탕으로 Gemini에게 개선 방법을 물어보세요.
          </p>
        )}
        {messages.map((m, i) => (
          <div
...
export default function InstrumentPage() {
  ...
  const [result, setResult] = useState<ReturnType<
    typeof demoInstrumentTuning
  > | null>(null)
  const [status, setStatus] = useState(
    '악기를 선택한 뒤 마이크로 연주를 녹음해 주세요.'
  )
          ...
          {catalog.map((item) => {
            ...
            return (
              <button
                ...
                className={`rounded-2xl border p-5 text-left transition-all hover:bg-muted/40 ${
                  active
              ...
              >
                <Icon
                  className={`mb-3 h-6 w-6 ${active ? 'text-primary' : ''}`}
                  aria-hidden
                />
                <p
        ...
        {result && (
          <section className="rounded-2xl border border-primary/20 bg-primary/5 p-6">
            ...
            <p className="mt-4 text-4xl font-semibold text-foreground">
              {result.tuningAccuracy}%
            </p>
            <p className="mt-1 text-sm text-muted-foreground">
              평균 편차 약 {result.pitchDeviationCents} cents ·{' '}
              {mic.durationSec}초 녹음
            </p>
            ...
            <ul className="mt-4 space-y-2">
              {result.stringReadings.map((row) => (
                ...
                >
                  ...
                  <span className="text-foreground">{row.cents} cents</span>
                </li>
              ))}
            </ul>
          </section>
        ...
        {result ? (
          ...
          <section className="rounded-3xl border border-border bg-card p-6">
            ...
            <p className="mt-3 text-sm leading-6 text-muted-foreground">
              튜닝 분석이 완료되면 Gemini에게 개선 방법과 연습 피드백을 바로
              받을 수 있습니다.
            </p>
          </section>
        )}
