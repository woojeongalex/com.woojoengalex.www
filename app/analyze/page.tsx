    ...
    <article className="rounded-3xl border border-border bg-card overflow-hidden">
      ...
      >
        {messages.length === 0 && !loading && (
          <p className="text-xs text-muted-foreground">
            분석 결과를 바탕으로 Gemini에게 코칭을 요청해 보세요.
          </p>
        )}
        {messages.map((m, i) => (
          <div
      ...
      {messages.length === 0 && (
        <div className="flex flex-wrap gap-2 px-4 pb-3">
          {['개선 방법 알려줘', '다음 연습 추천해줘', '점수 해석해줘'].map(
            (q) => (
              <button
                key={q}
                type="button"
...
export default function AnalyzePage() {
  ...
  const [recordingState, setRecordingState] = useState<
    'idle' | 'recording' | 'done'
  >('idle')
  const [inputSource, setInputSource] = useState<'none' | 'mic' | 'video'>(
    'none'
  )
  const [analysisResult, setAnalysisResult] =
    useState<VocalAnalysisResult | null>(null)
  ...
  const selectedSong = useMemo((): SongMrHit | null => {
    ...
    return (
      songFind.hits.find(
        (h) => h.catalog_song_id === songFind.selectedCatalogSongId
      ) ?? null
    )
  ...
  const handleSongSearch = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const rawQuery = formData.get('songQuery')
  ...
  const pickSong = (hit: SongMrHit) => {
    setSongFind((prev) => ({
      ...prev,
      selectedCatalogSongId: hit.catalog_song_id,
    }))
        ...
        <section className="rounded-2xl border border-border bg-muted/40 px-4 py-8 dark:bg-[#0d0d0d] sm:rounded-[2rem] sm:px-6 sm:py-10 md:px-10">
          ...
          <h1 className="mt-4 text-2xl font-semibold leading-snug tracking-tight sm:text-4xl lg:text-5xl">
            ...
            <span className="block">
              분석 결과를 하나의 화면에서 연결합니다.
            </span>
          </h1>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-muted-foreground sm:text-base">
            노래 제목으로 MR을 찾아 고르고, 마이크로 부르거나 연습 영상·음원을
            올려 AI가 음정과 박자를 분석한 뒤 코칭 피드백을 받을 수 있습니다.
          </p>
        </section>
            ...
            <article className="rounded-3xl border border-border bg-card p-6">
              ...
              >
                ...
                <button
                  ...
                  className="inline-flex shrink-0 items-center justify-center gap-2 rounded-2xl bg-primary px-6 py-3 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-80 disabled:opacity-50"
                >
              ...
              {!songFind.submittedQuery.trim() ? (
                <p
                  ...
                  role="status"
                >
                  노래 제목을 입력한 뒤 검색하면 등록된 MR 정보가 있는 곡 목록이
                  표시됩니다.
                </p>
              ) : searchLoading ? (
                <p
                ...
                <p
                  ...
                  role="status"
                >
                  &quot;{songFind.submittedQuery}&quot;에 맞는 곡을 찾지
                  못했습니다.
                </p>
              ) : (
                <div className="mt-6 grid grid-cols-1 gap-4 min-[420px]:grid-cols-2 lg:grid-cols-3">
                  {songFind.hits.map((song) => {
                    ...
                    return (
                      ...
                      >
                        <p
                          className={`text-base font-semibold ${active ? 'text-foreground' : ''}`}
                        >
                          {song.title}
                        </p>
                        ...
                        <div className="mt-4 space-y-1 text-xs text-muted-foreground">
                          ...
                          <p
                            className={`pt-2 text-xs font-medium ${active ? 'text-primary' : ''}`}
                          >
            ...
            <article className="rounded-3xl border border-border bg-card p-6">
              ...
              <div className="mt-6 rounded-2xl border border-border bg-muted/40 p-5 dark:bg-[#0d0d0d]">
                ...
                <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-80 disabled:opacity-40"
                  >
              ...
              <div className="mt-6 rounded-2xl border border-border p-5">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  커버 영상, 연습 녹화, MR 없는 클립 등에서 보컬 트랙을
                  분석합니다.
                </p>
                <VocalVideoDropzone
            ...
            <article className="rounded-3xl border border-border bg-card p-6">
              ...
              <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <ResultCard
                  ...
                  description={
                    analysisResult ? result.summary : '호흡과 발성 개선 제안'
                  }
                />
              </div>
              ...
              <div className="mt-6 rounded-2xl border border-border bg-muted/40 p-5 dark:bg-[#0d0d0d]">
                ...
                <p className="mt-3 text-sm leading-6 text-muted-foreground">
                  ...
                  {analysisResult?.fileName
                    ? ` · 입력: ${analysisResult.fileName}`
                    : ''}
                  {selectedSong
                    ? '의 원곡 BPM·키를 기준으로 '
                    : ' — MR을 선택하면 '}
                  {inputSource === 'video' ? '영상·음원' : '마이크'} 보컬과
                  비교합니다.
          ...
          <aside className="space-y-6">
                  ...
                  <dl className="mt-6 space-y-4 text-sm">
                    {[
                      ...
                      >
                        ...
                        <dd
                          className={`font-medium ${mono ? 'break-all font-mono text-xs' : ''}`}
                        >
                          {value}
                        </dd>
                      </div>
            ...
            {analysisResult ? (
              ...
              <article className="rounded-3xl border border-border bg-card p-6">
                ...
                <p className="mt-3 text-sm leading-6 text-muted-foreground">
                  분석이 완료되면 결과를 바탕으로 Gemini에게 개선 방법, 다음
                  연습 추천 등을 질문할 수 있습니다.
                </p>
              </article>
            )}
            ...
            {/* API 배너 */}
            <article className="rounded-3xl border border-primary/30 bg-primary/10 p-6">
              <div className="flex items-center gap-2 text-sm font-medium text-foreground">
              ...
              <div className="mt-6 space-y-3 text-sm">
                <ApiRow
                  label="MR 검색·DB 저장"
                  value={`${apiBaseUrl}/api/songs/search?q=`}
                />
                <ApiRow
                  label="보컬 분석 결과 저장"
                ...
                <ApiRow
                  label="녹음 업로드"
                  value={`${apiBaseUrl}/analysis/recordings`}
                />
              </div>
            </article>
          </aside>
