...
interface IntentDistRes {
  distribution: IntentDistItem[]
}
...
function Sparkline({ color, up }: { color: string; up: boolean }) {
  const pts = up
    ? '0,24 13,22 26,18 39,20 52,12 65,9 78,6'
...
export default function AdminPage() {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [s, ss, id, p, il] = await Promise.all([
        adminApiFetch<StatsRes>('/api/admin/stats'),
        adminApiFetch<ServiceStatsRes>('/api/admin/service-stats'),
        adminApiFetch<IntentDistRes>('/api/admin/intent-distribution'),
        adminApiFetch<PassengersRes>('/api/admin/passengers'),
        adminApiFetch<IntentLogsRes>('/api/admin/intent-logs'),
      ])
      setStats(s)
      setServiceStats(ss)
  ...
  const kpiCards = [
    {
      label: '총 음악 평가',
      value: num(stats?.totalEvals).toLocaleString(),
      color: C.blue,
      up: true,
    },
    {
      label: '평균 점수',
      value: `${num(stats?.avgScore)}점`,
      color: C.green,
      up: true,
    },
    {
      label: '인텐트 로그',
      value: num(stats?.intentLogCount).toLocaleString(),
      color: C.purple,
      up: true,
    },
    {
      label: '탑승객 데이터',
      value: num(stats?.passengerCount).toLocaleString(),
      color: C.amber,
      up: false,
    },
  ]
  ...
  const serviceBars = serviceStats
    ? [
        {
          label: '보컬 평가 처리율',
          value: serviceStats.vocalEvalRate,
          color: C.blue,
        },
        {
          label: 'AI 인텐트 분류 정확도',
          value: serviceStats.aiAccuracyRate,
          color: C.green,
        },
        {
          label: '탑승객 데이터 적재율',
          value: serviceStats.passengerDataRate,
          color: C.amber,
        },
        {
          label: '스피치 평가 완료율',
          value: serviceStats.speechEvalRate,
          color: C.purple,
        },
      ]
    : []
        ...
        <div style={{ padding: `24px ${px}px`, flex: 1, overflow: 'auto' }}>
                ...
                {kpiCards.map((kpi) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: isMobile ? '14px 14px 10px' : '18px 18px 14px',
                    }}
                  >
                    <div
                      style={{
                    ...
                    <div
                      style={{
                        ...
                        fontWeight: 800,
                        color: isLoading ? C.muted : C.black,
                        margin: '8px 0 6px',
                        letterSpacing: '-1px',
                        lineHeight: 1,
                      }}
              ...
              >
                ...
                >
                  ...
                  >
                    {isLoading
                      ? '—'
                      : num(stats?.intentLogCount).toLocaleString()}
                  </div>
                  <div
                    style={{
                      fontSize: 12,
          ...
          {active === 'intent-logs' && (
            ...
            >
              ...
              {isLoading ? (
                <div style={{ padding: '24px', fontSize: 13, color: C.muted }}>
                  불러오는 중...
                </div>
              ) : intentLogs.length === 0 ? (
                <div
                    ...
                    <tbody>
                      {intentLogs.map((log) => (
                        <tr
                          ...
                          style={{
                            background: hoveredRow === log.id ? C.bg : C.white,
                            borderBottom: `1px solid ${C.border}`,
                          }}
                        >
                          <td
                          ...
                          <td style={{ padding: '12px 16px', color: C.black }}>
                            {log.message}
                          </td>
            ...
            >
              ...
              >
                ...
                >
                  {isLoading
                    ? '—'
                    : `전체 ${passengerTotal.toLocaleString()}명 중 ${passengers.length}건`}
                </span>
              </div>
              {isLoading ? (
                <div style={{ padding: '24px', fontSize: 13, color: C.muted }}>
                    ...
                    <tbody>
                      {passengers.map((p) => (
                        <tr
                          ...
                          onMouseLeave={() => setHoveredRow(null)}
                          style={{
                            background: hoveredRow === p.id ? C.bg : C.white,
                            borderBottom: `1px solid ${C.border}`,
                          }}
                        >
                          <td
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            <span
                              style={{
                                ...
                                borderRadius: 4,
                                background:
                                  p.survived === 1 ? C.greenBg : C.redBg,
                          ...
                          <td style={{ padding: '12px 16px', color: C.black }}>
                            {p.age ?? <span style={{ color: C.muted }}>—</span>}
                          </td>
                          <td style={{ padding: '12px 16px', color: C.muted }}>
                            {p.fare != null ? `£${p.fare.toFixed(2)}` : '—'}
                          </td>
                          <td style={{ padding: '12px 16px', color: C.muted }}>
                            {p.embarked || '—'}
                          </td>
