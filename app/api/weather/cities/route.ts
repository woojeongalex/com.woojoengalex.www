...
export async function GET() {
  ...
  try {
    const res = await fetch(`${WEATHER_API_BASE}/api/weather/cities`, {
      cache: "no-store",
    ...
    if (!Array.isArray(data.cities) || data.cities.length === 0) {
      return NextResponse.json({ error: UI_ERRORS.weatherFailed }, { status: 502 })
    }
